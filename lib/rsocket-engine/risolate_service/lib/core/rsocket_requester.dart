...
class RSocketRequester extends RSocket {
  ...
  RSocketRequester(String mode, ConnectionSetupPayload connectionSetupPayload,
    ...
    if (this.connection.receiveHandler == null) {
      this.connection.receiveHandler = (chunk) => receiveChunks(chunk);
    }
  ...
  void sendSetupPayload() {
    ...
    if (mode == 'requester') {
      ...
      keepAliveTimer = Timer.periodic(
          Duration(days: connectionSetupPayload!.keepAliveInterval),
          (Timer t) {


        if (!closed) {
          connection.write(FrameCodec.encodeKeepAlives(false, 0));
        } else {
  ...
  Future<void> close() async {
    if (!closed) {
      ...
      _availability = 0.0;
      keepAliveTimer?.cancel();
      if(connection is WebSocketDuplexConnection) {
        await (connection as WebSocketDuplexConnection).closeConnection();
      } else {
        connection.close();
      }
    }
  ...
  void receiveChunks(Uint8List chunk) {
    ...
    switch (header.type) {
      ...
      case frame_types.KEEPALIVE:
        ...
        if (keepAliveFrame.respond) {
           connection.write(FrameCodec.encodeKeepAlives(
